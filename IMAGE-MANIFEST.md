...
|---|---|---|---|
| 01 | `pinch-pleat-drapery-living-room-orb-chandelier.jpg` | **Former Home hero — unused as of 2026-08-28** (replaced by frame 7, see below) | Cropped top-anchored from the original (full frame, trimmed ~250px off the bottom) to foreground the chandelier, drapery, and art rather than the seating group. Full chandelier visible, most generous headroom of the living-room set. |
...
| 7 | `living-room-archway-drapery-orb-chandelier.jpg` | **Home hero (set 2026-08-28)** + `/northern-kentucky` lead | **Sam supplied this exact frame and asked for it as the hero**, so the choice is his and not an inference from the file list. Pulled furthest back of the living-room angles: the columned archway frames the whole room, both drapery panels, the French doors and the chandelier in one shot, which suits a hero that carries no headline text (8.1). It also avoids frame 01's two known defects (intruding fig leaf at bottom-right, blown highlight down the right edge). **Note it is now doing two jobs** — Home hero and the `/northern-kentucky` lead. `/northern-kentucky` is a search-only page not linked from the main nav, so few visitors meet both, but if that duplication is unwanted the NKY lead is the one to change, not the hero. Earlier reasoning, still accurate: same archway room as frames 01/02, a third angle. Disqualified from portfolio as a near-duplicate of 08/09 with "nothing 8/17/18 didn't already cover" — a fair reason not to use three near-identical shots on one project page, but it doesn't disqualify the room itself from a completely different page. The sofa and chaise visible are confirmed **not** Amy's (same determination as everywhere else on this site); the drapery framing the French doors is. Disqualified from portfolio as a near-duplicate of 08/09 with "nothing 8/17/18 didn't already cover" — a fair reason not to use three near-identical shots on one project page, but it doesn't disqualify the room itself from a completely different page. The sofa and chaise visible are confirmed **not** Amy's (same determination as everywhere else on this site); the drapery framing the French doors is. |
